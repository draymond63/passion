from Passion.general import cluster, keys_to_str, combine_group
import pandas as pd
...
def name_tfidf_group(grouped_freqs: pd.DataFrame, grouped_titles: pd.Series, use_full_title=True) -> str:
    ...
    # Convert list to job title (string)
        ...
        keys = keys_to_str(keys, grouped_titles, use_full_title)
    
    # Append the job_keys
    return keys

# * Using clustering to append the cleaned dump with job key
def append_keys(appended_file='dump_cleaned.csv', input_file='tfidf/tfidf_positions.csv', title_col='posTitle', name_col='tfidfKey', num_col='tfidfKeyNum', cluster_threshold=1, use_full_title=True):
    ...
    for i, group in grouped_df:
        ...
        # Save the name
        job_keys[i] = name_tfidf_group(vecs, group_titles, use_full_title=use_full_title)
